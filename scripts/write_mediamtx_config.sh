...
if [[ -z "$INSTALL_DIR" ]]; then
  # 인자 없으면 스크립트 위치 기준으로 ../thirdparty/mediamtx 사용
...
# Disabled here: siyi_dashboard already uses port 8888, and the controller
# consumes video via WebRTC (port 8889) — HLS is not needed.
hls: no
...
paths:
  ...
  # RealSense depth stream (colorized rgb8 from realsense2_camera), pushed by image_bridge.
  depth:
    source: publisher
    sourceOnDemand: no
    overridePublisher: yes
